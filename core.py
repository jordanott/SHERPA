...
class _Runner(object):
    ...
    * Check if new trials need to be submitted, get parameters and submit as a
    ...
    """
    def __init__(self, study, scheduler, database, num_distributors, num_tasks_per_distributor,
                 command, resubmit_failed_trials=False, output_dir=''):
        ...
        self.num_tasks_per_distributor = num_tasks_per_distributor
        self._active_distributors  = []
    ...
    def update_active_distributors(self):
        for i in reversed(range(len(self._active_distributors))):
            status = self.scheduler.get_status(self._active_distributors[i])
            if status in [_JobStatus.finished, _JobStatus.failed, _JobStatus.killed, _JobStatus.other]:
                self._active_distributors.pop(i)
                    
    def update_active_trials(self):
        """
        Update active trials, finalize any completed/stopped/failed trials.
        """
        completed_trials_file_name = os.path.join(self._output_dir, 'completed_trials.json')
        ...
        with FileLock(completed_trials_file_name + ".lock"):
            try:
                ...
                with open(completed_trials_file_name, '') as f:
                    f.write('')
            except:
                pass
    ...
    def _get_batch_of_trials(self):
        batch = []
        for i in range(self.num_tasks_per_distributor):
            ...
            batch.append(next_trial)
            self._all_trials[next_trial.id] = next_trial
            self.database.enqueue_trial(next_trial)
            
        if batch:
            pid = self.scheduler.submit_job(
                command=self.command,
                env={'SHERPA_TRIAL_ID_MIN': str(batch[0].id),
                     'SHERPA_TRIAL_ID_MAX': str(batch[-1].id),
                     'SHERPA_OUTPUT_DIR': self.study.output_dir}
            )

            self._active_distributors.append(pid)
    ...
    def submit_new_trials(self):
        ...
        """
        while len(self._active_distributors) < self.num_distributors:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        for distributor in self._active_distributors:
            print('Killing trial', distributor)
            self.scheduler.kill_job(distributor)
...
def optimize(parameters, algorithm, lower_is_better,
    ...
    with _Database(db_dir=output_dir, port=db_port,
        ...
        runner = _Runner(study=study,
                         ...
                         num_tasks_per_distributor=num_tasks_per_distributor,
