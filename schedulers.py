...
Date: 5/20/2020
Description:
    Modify SLURMScheduler
    Remove drmaa dependency 
    libdrmaa.so.1.0 not present on Comet
    Call squeue and check for job status
...
class SLURMScheduler(Scheduler):
    ...
    Args:
        ...
        output_dir (str): path to directory in which ``stdout`` and ``stderr``
            ...
            defined for the study.
    """
    def __init__(self, submit_options, environment, output_dir='', username=''):
        self.count = 0
        self.submit_options = submit_options
        self.environment = environment
        self.output_dir = output_dir
    ...
    def submit_job(self, command, env={}, job_name=''):
        ...
        outdir = os.path.join(self.output_dir, 'distributors')
        if not os.path.isdir(outdir):
            os.mkdir(outdir)
            
        os.makedirs(
            os.path.join(self.output_dir, 'jobs'), 
            exist_ok=True
        )

        job_name = job_name or 'distributor_' + str(self.count)
        ...
        with open('template.csh', 'r') as f:
            run = f.read()
            
            for var_name, var_value in env.items():
                run = run.replace(var_name, var_value)
            
            with open('run.csh', 'w') as output:
                output.write(run)
        ...
        job_id = self._submit_job(submit_command, None)
...
#         output, std_err = process.communicate(input=run_command)
    ...
    def get_status(self, job_id):
        ...
        Returns:
            sherpa.schedulers._JobStatus: The job status.
        """
        queue = subprocess.check_output(['squeue','-u', self.username], stderr=subprocess.STDOUT)
        
        if str(job_id) not in str(queue):
            return _JobStatus.killed
        else:
            return None
#         with self.drmaa.Session() as s:
#             try:
#                 status = s.jobStatus(str(job_id))
#             except self.drmaa.errors.InvalidJobException:
#                 print("INVALID JOB EXCEPTION")
#                 return _JobStatus.finished
#         s = self.decode_status.get(status)
#         logger.info(s)
#         logger.info(_JobStatus.finished)
#         if s == _JobStatus.finished and job_id in self.killed_jobs:
#             s = _JobStatus.killed
#         logger.info(s)
#         return s
    ...
    def kill_job(self, job_id):
        ...
        """
        logger.info("Killing job {}".format(job_id))
        _=subprocess.check_output(['scancel', str(job_id)], stderr=subprocess.STDOUT)
#         with self.drmaa.Session() as s:
#             s.control(job_id, self.drmaa.JobControlAction.TERMINATE)
#         # TODO: what happens when job doesn't exist - then we don't want to add
#         self.killed_jobs.add(job_id)
